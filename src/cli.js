...
var OPTIONS = {
  ...
  "checkstyle-reporter": [
    "checkstyle-reporter",
    deprecated(
      "Use a CheckStyle compatible XML reporter",
      "--reporter=checkstyle"
    )
  ]
};
  ...
  if (config.overrides) {
    if (file) {
      _.each(config.overrides, function(options, pattern) {
        if (
          minimatch(path.normalize(file), pattern, {
            nocase: true,
            matchBase: true
          })
        ) {
...
var exports = {
  ...
  loadConfig: function(fp) {
    ...
    try {
      ...
      if (config["extends"]) {
        var baseConfig = exports.loadConfig(
  ...
  run: function(opts, cb) {
    ...
    function mergeCLIPrereq(config) {
      if (opts.prereq) {
        config.prereq = (config.prereq || []).concat(
          opts.prereq.split(/\s*,\s*/)
        );
      }
    }
    ...
    if (opts.useStdin && opts.ignores.indexOf(filename) === -1) {
      cli.withStdin(function(code) {
        ...
        (
          opts.reporter || defReporter
        )(results, data, { verbose: opts.verbose });
        cb(results.length === 0);
      });
      return null;
    }
