...
var OPTIONS = {
  ...
  "reporter" : [
    "reporter", "Custom reporter (<PATH>|jslint|checkstyle|unix)", "string",
    undefined
  ],
  "prereq" : [
    "prereq",
  ...
  "exclude" : [
    "exclude",
    "Exclude files matching the given filename pattern (same as .jshintignore)",
    "string", null
  ],
  "exclude-path" : [
    "exclude-path", "Pass in a custom jshintignore file path", "string", null
  ],
  "filename" : [
    "filename",
  ...
  "extra-ext" : [
    "e", "Comma-separated list of file extensions to use (default is .js)",
    "string", ""
  ],
  ...
  "checkstyle-reporter" : [
    "checkstyle-reporter",
    deprecated("Use a CheckStyle compatible XML reporter",
               "--reporter=checkstyle")
  ]
};
...
function getHomeDir() {
  ...
  var paths = [
    environment.USERPROFILE, environment.HOME, environment.HOMEPATH,
...
function extract(code, when) {
  ...
  function ontext(data) {
    ...
    if (startOffset) {
      lines =
          lines.map(function(line) { return line.replace(startOffset, ""); });
      data = lines.join("\n");
    }
  ...
  var parser = new htmlparser.Parser(
      {onopentag : onopen, onclosetag : onclose, ontext : ontext});
...
var exports = {
   ...
   */
  getConfig : function(
      fp) { return loadNpmConfig(fp) || exports.loadConfig(findConfig(fp)); },
  ...
  loadConfig : function(fp) {
    ...
    try {
      ...
      if (config['extends']) {
        var baseConfig =
            exports.loadConfig(path.resolve(config.dirname, config['extends']));
   ...
   * .jshintignores) extensions - A list of non-dot-js extensions to check
   */
  gather : function(opts) {
    ...
    var ignores = !opts.ignores ? loadIgnores({cwd : opts.cwd})
                                : opts.ignores.map(function(
                                      target) { return path.resolve(target); });

    opts.args.forEach(function(
        target) { collect(target, files, ignores, reg); });
  ...
  run : function(opts, cb) {
    ...
    function mergeCLIPrereq(config) {
      if (opts.prereq) {
        config.prereq =
            (config.prereq || []).concat(opts.prereq.split(/\s*,\s*/));
      }
    }
   ...
   * Used to determine is stdout has any buffered output before exiting the
   * program
   */
  getBufferSize : function() { return process.stdout.bufferSize; },
  ...
  interpret : function(args) {
    ...
    function done(passed) {
      ...
      reporter : reporter,
      ignores : loadIgnores(
          {exclude : options.exclude, excludePath : options["exclude-path"]}),
